...
export function DetailPanel({ summary, onClose, onDelete }: DetailPanelProps) {
  ...
  const hue = Math.round(
    180 + (Math.min(Math.max(summary.compressionRatio, 0), 100) / 100) * 90,
  );
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.metaBlock}>
        ...
        <span className={styles.sourceId}>#{summary.id}</span>
      </div>
      <div className={styles.grid}>
        {[
          ...
          {
            label: "Compressed",
            value: summary.compressionRatio.toFixed(0) + "%",
            unit: "",
          },
        ].map((s) => (
          <div key={s.label} className={styles.gridContainer}>
            <div
            ...
            >
              ...
              <span style={{ fontSize: 10, color: "8B8FA8", marginLeft: 2 }}>
                {s.unit}
              </span>
            </div>
            <div className={styles.gridLabel}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className={styles.footer}>
        <span className={styles.footerText}>Saved {summary.savedAt}</span>
      </div>
    </div>
  );
}
